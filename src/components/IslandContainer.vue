  ...
  setup() {
    ...
    function initThreeJS() {
      ...
      function onPointerMove(event) {
        if (
          currentDragIsland &&
          currentDragIsland.isDragging &&
          currentDragIsland.dragStart
        ) {
          ...
          const newRotationY =
            currentDragIsland.dragStart.rotationY + deltaX * dragSensitivity;
          const newRotationX =
            currentDragIsland.dragStart.rotationX + deltaY * dragSensitivity;
          const currentTime = performance.now();
          const deltaTime =
            currentTime - currentDragIsland.dragStart.time || 16;
          const newVelocityX =
            ((deltaY * dragSensitivity) / (deltaTime / 1000)) * momentumFactor;
          const newVelocityY =
            ((deltaX * dragSensitivity) / (deltaTime / 1000)) * momentumFactor;
          currentDragIsland.rotationVelocity = {
            x: newVelocityX,
            y: newVelocityY,
          };
          currentDragIsland.dragStart.pointerX = event.clientX;
          currentDragIsland.dragStart.pointerY = event.clientY;
          currentDragIsland.dragStart.rotationX =
            currentDragIsland.mesh.rotation.x;
          currentDragIsland.dragStart.rotationY =
            currentDragIsland.mesh.rotation.y;
    ...
    function createIslands() {
      ...
      if (isMobile.value) {
        ...
        positions = [
          ...
          { x: 0, y: -4.3, z: 1, rotX: Math.PI / 15, rotY: 0, rotZ: 0 },
        ];
        islandFiles = [
          "/aspan_web/assets/islandTennis.glb",
        ...
        loader.load(
          ...
          (gltf) => {
            ...
            islands.push({
              ...
              dragStart: null,
              autoRotationSpeed:
                autoRotationSpeed + Math.random() * autoRotationVariation,
            });
          },
          undefined,
